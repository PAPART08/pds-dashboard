...
export default function NepProjectTrackerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center space-x-4 mb-4">
        <Link
          href={isMember ? "/dashboard/user-task" : "/dashboard/nep/review"}
        ...
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: 'var(--dpwh-green, #10b981)' }}>NEP Project {project?.alternateId || id.substring(0, 8).toUpperCase()}</h1>
            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700 uppercase">{project?.status || 'NEP Draft'}</span>
