...
export default function ReviewQueuePage() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.from('projects').select('*').eq('phase', 'NEP').eq('is_epa', true);
  ...
  return (
    <div className="space-y-6 animate-fade-in px-4 md:px-0 pb-10">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight mb-1" style={{ color: 'var(--dpwh-green, #10b981)' }}>NEP Technical Review Queue</h1>
        <p className="text-[color:var(--text-muted)] text-sm">Perform technical checking and verify EPA program items.</p>
      </div>
                ...
                projects.map((item, idx) => (
                  <tr key={idx} className="hover:bg-blue-50/20 transition-colors group">
                    ...
                    <td className="p-6 text-right whitespace-nowrap">
                      <Link href={`/dashboard/nep/${item.id}`} className="inline-flex items-center space-x-2 bg-[color:var(--dpwh-green)] text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md hover:bg-green-800 transition-all active:scale-95">
