...
export default function ReviewQueuePage() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.from('projects').select('*').eq('phase', 'GAA');
  ...
  return (
    <div className="space-y-6 animate-fade-in px-4 md:px-0 pb-10">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight mb-1" style={{ color: 'var(--dpwh-orange, #f97316)' }}>GAA Technical Review Queue</h1>
        <p className="text-[color:var(--text-muted)] text-sm">Perform technical checking and verify GAA program items.</p>
      </div>
