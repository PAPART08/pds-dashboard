...
import styles from '../../rbp/master-list/page.module.css'; // Reusing global list styles
...
export default function GAAProjectsList() {
  // --- State ---
  ...
  const fetchProjects = async () => {
    setIsLoading(true);
    try {
      // Fetch exclusively from Supabase where phase='GAA'
      ...
      console.error('Error fetching GAA projects:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, altId?: string) => {
    if (!(await window.customConfirm(`Are you sure you want to delete GAA project ${altId || id}? This will revert it back to the NEP phase migration queue.`))) return;

    try {
      // 1. Revert the parent NEP project's status back to 'Approved' so it can be migrated again
      if (altId) {
        await supabase
          .from('projects')
      ...
      // 2. Delete the GAA project clone
  ...
  const handleExportMYPS = async () => {
    ...
    try {
      ...
      const { data: projs, error: pError } = await supabase.from('projects').select('*').in('id', projectIds);
      if (pError) throw pError;

      const { data: comps, error: cError } = await supabase.from('project_components').select('*').in('project_id', projectIds);
      if (cError) throw cError;

      const { data: specs, error: sError } = await supabase.from('project_infra_activities').select('*').in('project_id', projectIds);
    ...
    <div className={styles.container} onClick={closeAllDropdowns}>
      <div className={styles.headerSection}>
        <div className={styles.headerTitleBox}>
          <h1 className={styles.headerTitle} style={{color: 'var(--dpwh-orange)'}}>All GAA Projects</h1>
          <p className={styles.headerSubtitle}>Comprehensive tracking of the enacted General Appropriations Act projects.</p>
        </div>

        <div className={styles.headerActions} style={{ position: 'relative' }}>
          <button
            className={styles.btnPrimary} style={{ backgroundColor: 'var(--dpwh-orange)' }}
          ...
          {isExportDropdownOpen && (
            <div className={styles.dropdownMenu} style={{ right: 0, left: 'auto', minWidth: '180px', marginTop: '0.5rem' }}>
              <button className={styles.dropdownItem} onClick={handleExportMYPS}>
      ...
      <div className={styles.glassCard}>
        <div className={styles.toolbar}>
          <div className={styles.searchBox} onClick={e => e.stopPropagation()}>
            ...
            <input
              ...
              placeholder="Search GAA Projects by ID or Title..."
              value={searchTerm}
          ...
          <div className={styles.filterGroup} onClick={e => e.stopPropagation()}>
            ...
            {(searchTerm || statusFilter !== 'All' || locationFilter !== 'All' || yearFilter !== 'All') && (
               <button
                 ...
                 onClick={() => { setSearchTerm(''); setStatusFilter('All'); setLocationFilter('All'); setYearFilter('All'); }}
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th className={styles.th}>GAA Project Details</th>
                <th className={styles.th}>Location</th>
                <th className={styles.th}>Final Cost (PHP)</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className={styles.td} style={{ textAlign: 'center', padding: '3rem 1rem' }}>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>
                      {projects.length === 0
                        ? "No GAA projects found. Start by migrating projects from NEP."
                ...
                paginatedProjects.map((project) => (
                  <tr key={project.id} className={styles.tr}>
                    ...
                    <td className={styles.td}>{getStatusBadge(project.status)}</td>
                    <td className={styles.td} style={{ textAlign: 'right' }}>
                      <div className={styles.actionBtns} style={{ justifyContent: 'flex-end' }}>
                        {isWorkspaceUser ? (
                          ...
                          <>
                            <Link href={`/dashboard/rbp/new?id=${project.id}&phase=GAA`} className={styles.actionBtn} title="Edit Project Details">
                              <span className={`material-symbols-outlined ${styles.actionIcon}`}>edit</span>
                            </Link>
                            <Link href={`/dashboard/rbp/${project.id}?phase=GAA`} className={styles.actionBtn} title="View Details">
                              <span className={`material-symbols-outlined ${styles.actionIcon}`}>visibility</span>
                            </Link>
                            <button className={`${styles.actionBtn} ${styles.actionBtnDelete}`} title="Delete GAA Project" onClick={() => handleDelete(project.id, project.alternateId)}>
          ...
          <div className={styles.pagination}>
            ...
            <div className={styles.pageControls}>
              ...
              {getPageNumbers().map(pageNum => (
                <button key={pageNum} className={`${styles.pageBtn} ${currentPage === pageNum ? styles.active : ''}`} onClick={() => handlePageChange(pageNum)}>{pageNum}</button>
              ))}
